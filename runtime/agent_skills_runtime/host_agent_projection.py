...
def _role_prompt(role: MultiAgentRole) -> str:
    ...
    return (
        ...
        "Do not ask the user to choose ordinary implementation details. Resolve rule-defined, recoverable, conventional, defaulted, and low-risk reversible choices yourself. "
        "If a true material owner decision is required, return it to the parent under PARENT_DECISION instead of asking the user directly. "
        "Return missing authorization, required user input, or capability blockers to the parent rather than prompting the user directly. "
