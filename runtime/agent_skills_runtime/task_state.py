...
def public_task_state_contract() -> dict[str, Any]:
    """返回宿主维护/恢复长任务状态所需的最小公共字段契约。"""
    return {
        "协议": TASK_STATE_PROTOCOL,
        "字段": list(_PUBLIC_FIELDS),
        "已完成切片字段": ["切片", "结果", "证据", "修订"],
        "维护时机": ["阶段实质变化", "上下文压缩前", "任务交接前", "宿主重连前"],
        "边界": "状态只保存已确认问题求解事实，不产生权限、路由、验证通过或完成结论。",
    }
