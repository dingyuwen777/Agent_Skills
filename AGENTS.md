...
本文件只指导 AI **开发、审查、测试、交付和维护 Agent_Skills 源仓库本身**。它不是最终用户说明，也**不得复制到目标项目**作为项目规则。

目标项目真正的 Agent Skills 使用入口由 Runtime 安装生成的 `AGENTS.md` managed block、Native Core Skill 和项目 MCP 共同承担；最终人类使用说明只有根 `USAGE.md`。
...
1. 先读本文件；
2. 再读 `.agents/skills/coding/SKILL.md`，按四维任务路由选择当前真正命中的 references；
3. 修改 Review、Docs、Figma 时，再读取对应 `SKILL.md` 与任务直接相关 references；
4. 规则迁移、拆分、通用化、删文档或调整 Ownership 时，必须读取 `coding/references/16_规则内容守恒与Skill维护.md`；
5. Runtime / Project Payload / Bundle / Stub / 项目安装 / MCP / Release 变化时，必须读取 `coding/references/14_本地MCP_Runtime分发与原文上下文加载.md`；
6. 不从历史聊天或其他业务仓库猜当前实现，以当前分支真实文件、GitHub 状态和本轮运行结果为准；
7. 只读取当前任务直接相关的代码、规则、测试、Workflow 和配置，不机械通读所有 references。
...
当前名称只是仓库事实，不是分发白名单。正式 Skill 必须始终从：
...
```text
Coding
→ 通用研发、调试、验证、Change、Git/CI/交付与跨 Skill 路由

Review
→ 独立审查、Findings、测试充分性与 re-review

Docs
→ 技术文档事实同步、审查、编写与更新

Figma
→ 设计事实、Canvas/Prototype、设计系统/可用性、Ready 与 Design-to-Code 交接
...
任何 Skill/Reference/模板/managed block 的拆分、合并、迁移、删 README、通用化或“精简”都必须保证：
...
- 原本由辅助 README 承担但仍属正式规则的内容，必须先证明已在 `SKILL.md` / references / managed block 中可达，才能删除 README；
...
- Payload 排除 canonical Reference 正文、tests 和维护 README；
...
→ ownership / AGENTS / host config / rollback
