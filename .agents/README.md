...
Review 不维护第二套 Coding 规范；Docs 也不复制 Coding 的研发规则。正常路由是：
...
## 5. Agent Skills Bootstrap

把三个 Skill 安装到目标项目后，推荐通过 Coding Bootstrap 建立稳定入口：
...
已有目标项目 `AGENTS.md` 时，只有 Agent Skills managed markers 内由 Bootstrap 管理；marker 外项目原文必须保持。完整安装与 Bootstrap 规则见：
...
- 仓库整体安装、Full/Runtime 分发和 Release：[`../README.md`](../README.md)

不要机械读取所有 references；由对应 `SKILL.md` 的触发条件决定当前任务真正需要加载哪些规则。
