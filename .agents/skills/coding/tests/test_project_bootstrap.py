...
def _load_module(name: str, path: Path):
    """从指定路径加载 Coding 模块，便于验证确定性 Bootstrap 行为。"""
...
class ProjectBootstrapTest(unittest.TestCase):
    ...
    def test_bootstrap_creates_agents_for_greenfield_without_inventing_stack(self) -> None:
        """空项目应创建可用 AGENTS 初版，并保持项目侧 Bootstrap 而不写内部披露说明。"""
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("当前真实文件", content)
            self.assertIn("首次接入", content)
            self.assertIn("完整性无法确认", content)
            self.assertIn("本区块由安装/升级流程维护", content)
            for forbidden in (
                "研发治理 MCP",
                "Runtime Mode",
                "Source Mode",
    ...
    def test_bootstrap_preserves_existing_agents_bytes_outside_managed_block(self) -> None:
        """已有 AGENTS 原文必须逐字保留，只允许追加 Agent Skills 自管区。"""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self._install_minimal_coding(root)
    ...
    def test_bootstrap_updates_only_existing_managed_block(self) -> None:
        """已有完整 managed block 时只替换自管区，marker 前后用户文本保持原样。"""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self._install_minimal_coding(root)
            prefix = b"# Existing\n\nuser-before\n"
            ...
            (root / "AGENTS.md").write_bytes(
                prefix + b"<!-- agent-skills:managed:start -->\nOLD BLOCK\n<!-- agent-skills:managed:end -->" + suffix
            )
    ...
    def test_bootstrap_is_idempotent(self) -> None:
        """连续执行 Bootstrap 不得重复追加 managed block 或缓存 ignore。"""
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            self._install_minimal_coding(root)
            CODING.bootstrap_project(root)
