...
class FrontendDesignToCodeRulesTest(unittest.TestCase):
    ...
    def test_greenfield_prefers_vue_without_overriding_constraints(self) -> None:
        """无既定框架的新前端应首选推荐 Vue，但约束不匹配时仍需比较真实备选。"""
        reference = REFERENCE_PATH.read_text(encoding="utf-8")
        for marker in (
            "Greenfield",
            "Vue",
    ...
    def test_readme_explains_the_entry_point_without_copying_the_full_rule(self) -> None:
        """人类 README 应给出使用入口和 Greenfield 默认边界，但正式细节仍归 reference。"""
        readme = self._read("README.md")
        self.assertIn("Design-to-Code", readme)
        self.assertIn("Greenfield", readme)
        self.assertIn("Vue", readme)
        self.assertIn("17_前端与Design-to-Code实施规则.md", readme)
