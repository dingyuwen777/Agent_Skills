...
class UniversalFigmaSkillTest(unittest.TestCase):
    """验证 Figma Skill 完整迁入、通用化与跨 Skill 路由不会丢失高价值约束。"""
    ...
    def test_formal_figma_skill_keeps_complete_reference_structure(self) -> None:
        """正式 Figma Skill 必须保留主文件、元数据和 00–07 全部专项 reference。"""
        expected = {
            "00_通用适用性与项目形态.md",
            "01_事实源与审查流程.md",
            "02_业务能力与真实系统映射.md",
        ...
        self.assertTrue((FIGMA_ROOT / "README.md").is_file())
    ...
    def test_high_value_figma_rules_remain_actionable(self) -> None:
        """Canvas、Prototype、Owner、状态、Ready、失败处理和写后复核不能被摘要掉。"""
    ...
    def test_figma_skill_does_not_embed_aima_business_facts(self) -> None:
        """Figma live 规则不能携带 AIMA 业务仓库、Provider、Stage 或 Blueprint 事实。"""
    ...
    def test_coding_routes_figma_review_to_figma_and_ready_code_to_reference_17(self) -> None:
        """设计审查必须归 Figma，READY 后的生产实现仍归 Coding reference 17。"""
    ...
    def test_bootstrap_and_navigation_expose_figma_without_static_distribution_whitelist(self) -> None:
        """项目入口必须认识 Figma，同时继续声明正式 Skill 由目录动态发现。"""
        ...
        agents_readme = self._read(ROOT / ".agents/README.md")
        for text in (managed, root_agents, root_readme, agents_readme):
            self.assertIn("figma", text.lower())
        for text in (root_agents, root_readme, agents_readme):
            self.assertIn(".agents/skills/*/SKILL.md", text)

    def test_real_repository_distribution_discovers_figma_automatically(self) -> None:
        """新增正式 Figma Skill 必须自动进入 Bundle、公开 Catalog 和 Project Payload。"""
