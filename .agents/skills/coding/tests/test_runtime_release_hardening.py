...
class RuntimeReleaseHardeningTest(unittest.TestCase):
    ...
    def test_owned_codex_table_without_managed_marker_fails_closed(self) -> None:
        existing = b"[mcp_servers.agent-skills]\n" b'command = ".agents/runtime/agent-skills"\n' b'args = ["serve"]\n'
        with self.assertRaisesRegex(ValueError, "marker|同名 MCP server"):
            INSTALLER._updated_codex_config(existing, ".agents/runtime/agent-skills", True)
    ...
    def test_release_workflow_uses_no_immutability_gate_or_custom_admin_secret(self) -> None:
        workflow = RELEASE_WORKFLOW.read_text(encoding="utf-8")
        for removed_gate in (
            "secrets.RELEASE_SETTINGS_TOKEN", "RELEASE_SETTINGS_TOKEN", "immutable-releases",
            "immutable_status", "Release Immutability", "Administration: read", ".immutable",
        ):
            self.assertNotIn(removed_gate, workflow)
        for preserved_gate in (
            'GH_TOKEN: ${{ github.token }}', 'git/ref/tags/${TAG}', 'gh release view "${TAG}"',
            "python -m unittest discover", "ready_check.py --root .", "Create verified draft Release",
            "gh release upload", "--draft=false",
        ):
    ...
    def test_failed_release_job_cleans_only_unpublished_draft(self) -> None:
        workflow = RELEASE_WORKFLOW.read_text(encoding="utf-8")
        for required in (
            "Cleanup failed draft Release", "if: failure()", "--json isDraft",
            'if [ "${is_draft}" = "true" ]', "gh release delete", "--cleanup-tag", "--yes",
        ):
            self.assertIn(required, workflow)
    ...
    def test_builder_reports_context_footprint_without_reference_details(self) -> None:
        ...
        for skill in skills:
            ...
            self.assertEqual(
                ...
                budget["entry_bytes"] + budget["router_bytes"] + (0 if skill == "router" else budget["skill_core_bytes"][skill]),
            )
            expected_reference_bytes = sum(
                int(entry["size"]) for entry in bundle["references"] if str(entry["skill"]) == skill
            )
