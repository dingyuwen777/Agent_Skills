...
INTENTIONAL_REQUIRED_REMOVALS = {
    ...
    "Figma → Code": {"coding.reference.04", "coding.reference.10", "coding.reference.19"},
    "Docs targeted": {"coding.reference.19"},
}
INTENTIONAL_REQUIRED_ADDITIONS = {
    "L1 mechanical": {"coding.reference.21"},
    "Docs targeted": {"coding.reference.21"},
}
INTENTIONAL_FIXED_CONTEXT_GROWTH = {
    # Repository L1 路由补充会让所有包含 ref02 的复杂高风险组合增加少量导航文本；
    ...
    def test_legacy_routes_preserve_safety_except_explicit_progressive_disclosure_changes(self) -> None:
        ...
        for case in baseline["cases"]:
            with self.subTest(case=case["name"]):
                ...
                additions = INTENTIONAL_REQUIRED_ADDITIONS.get(case["name"], set())
                self.assertTrue(additions.issubset(actual_references), additions - actual_references)
