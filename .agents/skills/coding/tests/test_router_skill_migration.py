...
class RouterSkillMigrationTest(unittest.TestCase):
    ...
    def test_legacy_routes_do_not_lose_or_wrongly_add_required_context(self) -> None:
        """固定旧 main 基线，迁移后欠披露、禁止误披露和风险降级均为零。"""
        baseline = json.loads(BASELINE_PATH.read_text(encoding="utf-8"))
        self.assertEqual(baseline["schema"], "agent-skills-router-legacy-baseline/v1")
        self.assertEqual(
            baseline["source_commit"],
            "3d22ca4e71bd7fcc83e35fd48abeae8eec00dd5e",
        )
        ...
        for case in baseline["cases"]:
            with self.subTest(case=case["name"]):
                ...
                self.assertTrue(
                    set(case["required_references"]).issubset(actual_references),
                    set(case["required_references"]) - actual_references,
                )
                self.assertFalse(set(case["forbidden_references"]) & actual_references)
                self.assertEqual(actual["最低风险"], case["minimum_risk"])

                current_bytes = (
                    int(budget["entry_bytes"])
                    + sum(int(budget["skill_core_bytes"][skill]) for skill in actual_skills)
                    + sum(reference_sizes[reference] for reference in actual_references)
                )
                self.assertLessEqual(
                    current_bytes,
                    int(case["context_bytes"]) + CONTEXT_GROWTH_LIMIT,
                    f"{case['name']} 上下文增长超过迁移和 Anti-Agent 边界的 8 KiB 预算",
                )
    ...
    def test_router_has_anti_agent_boundary_and_runtime_exposes_no_executor(self) -> None:
        ...
        self.assertFalse(
            {name for name in public_functions if any(word in name for word in ("plan", "execute", "agent"))}
        )
        ...
        self.assertEqual(
            result_keys,
            {"路由摘要", "命中Skill", "必需Reference", "最低风险", "存在未知项"},
        )
