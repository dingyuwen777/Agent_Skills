...
class TestingRuntimeProjectionTest(unittest.TestCase):
    """锁定 Testing Runtime Projection 的专业语义与 project-facing 明文边界。"""
    ...
    def test_testing_projection_preserves_professional_owner_semantics(self) -> None:
        """project-facing 投影不能删除 Testing 的专业测试方法、黑盒、Regression 与缺陷回程语义。"""
        for marker in (
            "唯一负责的专业方法",
        ...
        for forbidden in ("Handoff", "Coding", "Skill", "Reference", "Router"):
            with self.subTest(forbidden=forbidden):
                self.assertNotIn(forbidden, self.runtime_testing)

    def test_testing_projection_hides_all_canonical_reference_identities(self) -> None:
        """Testing Runtime Core 不得暴露任一 canonical Reference 文件名、路径或 Stable ID。"""
