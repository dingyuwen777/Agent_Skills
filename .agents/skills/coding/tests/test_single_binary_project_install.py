...
class SingleBinaryProjectInstallTest(unittest.TestCase):
    ...
    def test_first_install_is_project_scoped_and_preserves_project_owned_content(self) -> None:
        ...
        runtime_router = (self.target / ROUTER_RELATIVE).read_text(encoding="utf-8")
        self.assertIn("Project Engineering Guardrails", runtime_router)
        self.assertIn("当前项目", runtime_router)
        self.assertIn("Fresh Evidence Contract", runtime_router)
        self.assertNotIn("agent_skills_load_required_context", runtime_router)
        self.assertNotIn("agent-routing:v1", runtime_router)
