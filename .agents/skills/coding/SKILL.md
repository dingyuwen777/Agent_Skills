...
17. **系统级分析先于局部实现。** 分析、方案、诊断和重要实现决策先沿当前问题真实能力链恢复调用关系、数据/状态、能力 Owner、现有实现与消费者，再决定局部修复、复用、公共抽象或能力归一；不能因为症状出现在单个文件就把系统问题局部化。系统级分析保持任务相关有界，不等于机械扫描全仓；详细方法见 [05_设计实施与根因调试.md](references/05_设计实施与根因调试.md)。
18. **受影响代码域必须整洁收口。** 持久代码修改完成时，对本次实际触及的文件、模块和直接相邻责任域检查并清理已确认失效、重复、无引用且无隐式语义的代码与辅助实现，使修改后的整体仍清晰易读；清理必须保护兼容、动态引用和用户工作，不能借机扩大成全仓无关重构。安全删除与收口细节见 [05_设计实施与根因调试.md](references/05_设计实施与根因调试.md)。
