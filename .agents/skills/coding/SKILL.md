...
<!-- agent-routing:v1
{
  ...
  "Skill": "coding",
  "触发": {"任一": [
    {"包含": {"维度": "执行模式", "取值": ["诊断", "Git", "发布", "运维"]}},
    {"包含": {"维度": "阶段", "取值": ["仓库初始化", "事实恢复", "需求设计", "功能开发", "缺陷修复", "重构", "性能优化", "故障处置", "交付"]}},
    {"包含": {"维度": "意图", "取值": ["代码分析", "技术方案", "代码实现", "代码审查", "设计转代码", "Git 交付", "依赖升级", "Runtime 升级", "Skill Mutation"]}}
  ]}
}
-->
...
Coding 继承 Router 的统一语义，并把它落实为研发默认动作：

```text
事实恢复 / 核验
→ 默认由 Agent 自行读取当前请求、仓库、代码、Manifest/lock、Contract、Schema、测试、CI、工具与一手资料
→ 能查出的事实不问用户

提请用户 / Owner 决策
→ 有界调查仍无法确定
→ 且答案会实质改变业务/public Contract/数据/安全/不可逆/重大技术路线
→ 只问最上游一个问题
```

除非当前条款明确要求“提请用户 / Owner 决策”“批准”或等价审批，否则“确认、明确、确定、恢复、核对”都表示**默认由 Agent 自行**完成事实核验。用户当前请求或正式事实源已经固化的决定**不重复确认**；新事实使原决定失效时才重新进入决策 Gate。

**阻塞按依赖边界传播**：本地终端不可用不妨碍能够通过仓库 API 完成的只读事实恢复；缺少 merge 权限不妨碍已授权的实现、测试和 PR Ready；某一外部 Probe 不可用不妨碍与它无关的稳定回归。只停止依赖 blocker 的动作与完成声明，并明确已完成范围、阻塞动作、缺失 Evidence/权限和仍可继续的安全范围。最终 `complete / mergeable / releasable / deployable` 仍必须满足各自全部 required gate。
