...
- 不能执行脚本/测试：按人工流程继续，明确未验证项，不伪造脚本结果；其他已授权 Evidence 通路可用时继续；
...
编码前明确背景/事实、目标、可观察成功标准、范围/非目标、不变项、输入输出、影响边界、复用点、预计文件、兼容/数据/Migration/依赖、验证、文档、部署/回滚（适用时）和 Git 授权。
...
只有仓库和正式资料无法确认、且会实质改变业务语义/验收、public API/ABI/CLI/格式/Contract、Schema/Migration/数据、权限/隐私/安全、外部 Provider 费用、SLO/RPO/RTO、破坏性兼容、不可逆操作或重大技术路线时才**提请用户 / Owner 决策**：先查事实和一手资料，给推荐与必要方案比较，只问最上游问题，再同步正式事实源和 Change。已经固化的决定**不重复确认**；普通可逆实现细节和能直接核验的事实不形成额外审批点。
...
每个完成结论都重新确定能证明结论的命令/检查，实际运行完整命令，读取输出/退出码/失败数量，对照成功标准、Traceability（适用时）、Validation Matrix 和 diff，只陈述证据支持的状态。历史日志、子 Agent 报告或“看起来正确”不能替代本轮新鲜证据。
