...
Change 的 Repository Ownership 由“谁被修改 / 谁被治理”决定，不由“规则从哪里加载”决定。每个 Coding Change 或项目等价持久施工契约必须只属于一个**唯一被治理仓库**。所有 carrier 路径、`affected_paths`、`contracts`、`data_changes` 以及仓库相对 Evidence 都相对于该仓库根解释；不得借另一个仓库的 Change carrier 承载当前仓库的施工范围。
