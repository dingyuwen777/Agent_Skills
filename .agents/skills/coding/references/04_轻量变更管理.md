<!-- agent-routing:v1
{"协议":"Agent Skills Reference路由/v1","标识":"coding.reference.04","触发":{"任一":[{"包含":{"维度":"风险","取值":["L2","L3"]}},{"包含":{"维度":"治理","取值":["存在活动变更","多个活动变更","要求变更记录"]}}]},"依赖":["coding.reference.02"]}
-->
...
把一个重要变化作为一个可独立理解、实现和验收的工作单元。复杂度通过同一施工契约的章节深度增加，不通过固定生成 proposal、spec、design 和 tasks 四套文件增加。
...
- L2：新功能、业务行为变化、重要 Bug、多文件修改、多人并行或需要审计。必须有一个可审计施工契约。
- L3：公共 API/ABI/CLI/数据格式、Schema、Migration、跨模块 Contract、架构、认证授权、安全、部署、重大依赖或破坏性兼容变化。仍使用一个施工契约，但补充方案比较、Migration、部署、回滚和风险。

发现隐藏影响时把 L1 升为 L2、把 L2 升为 L3。不要为了少写文档而降级。

## 先发现项目已有治理，不制造平行制度

L2/L3 在创建 Coding 自带 Change 前，先检查目标仓库是否已有正式变更治理，例如：
...
就复用项目已有载体，不为了 Coding 再创建平行 Change。

如果已有机制不能承载这些语义：

- 不静默降低 Coding 的需求追溯、验证和 Completion Audit 门禁；
- 先按项目规则判断是否允许增加最小附属记录；
- 需要实质改变项目治理时按上游设计决策处理；
- Coding CLI 不猜 OpenSpec/RFC 等第三方 schema，也不自动改写它们。

项目没有可复用治理机制时，才使用 Coding 自带 Change。
...
它表示该 Change 在进入 Ready/Archive 时必须通过 Requirement Traceability 与 Completion Audit 门禁。
...
每个新的 Coding Change，或项目等价施工契约，至少写清：
...
Greenfield L2/L3 也遵守同样结构，但“必须保持不变”可以来自用户已确认约束、预期平台、兼容目标和组织政策；不存在的旧 API/Schema 不要伪造为兼容对象。
...
对 L2/L3，在实现前根据当前任务的真实失败边界选择验证层，并在完成前补新鲜证据。
...
进入 `ready_for_review` / 项目等价 Ready 前，不先按当前 Change 的 checklist 验收，而是重新读取上游事实源并执行：
...
- 完成前改变同一目标：先回到上游要求，更新 Traceability，再更新当前 Change/项目等价施工契约；在关键决策中记录重要取舍。
