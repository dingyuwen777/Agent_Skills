<!-- agent-routing:v1
{"协议":"Agent Skills Reference路由/v1","标识":"coding.reference.10","触发":{"任一":[{"包含":{"维度":"风险","取值":["L2","L3"]}},{"包含":{"维度":"治理","取值":["要求完成门禁"]}},{"包含":{"维度":"阶段","取值":["交付"]}}]},"依赖":["coding.reference.04","coding.reference.07"]}
-->
...
它适用于不同项目形态和语言；“Stage”只是常见的正式工作单元之一，也可以是 Feature、Migration、Release、Architecture Change、Greenfield Bootstrap 或其他有明确上游完成定义的 L2/L3 单元。
...
`CHANGE.md` 是施工契约，不是自身需求全集。不能用“当前 Change 的成功标准全部完成”证明“上游正式单元已完整完成”。项目使用 OpenSpec、RFC/ADR 或其他治理载体时同理：当前施工记录不能反过来成为自己的需求全集。

## Requirement Traceability

Coding 自带的新 L2/L3 Change 使用：
...
进入 `ready_for_review` / 项目等价 Ready 状态前必须重新读取上游事实源，不先看当前 Change 的 checklist 来反推需求。至少完成：
