...
仅在存在异步加载、可切换对象或可编辑表单时检查以下边界；复用当前框架与已有请求/状态 Owner，不为这些规则引入新的状态库。

- 请求发出时固定对象身份、必要参数和请求代次。响应提交前确认仍属于当前对象与有效请求；A → B → A 的切换不能只靠 ID 相等判断旧响应有效。复用现有取消、请求序号或等价机制即可，不机械叠加全部方案。
- pending / error / result 的作用域匹配操作对象；多对象独立操作时不能用一个无归属的全局布尔值串状态。过期请求的 finally 也不能清掉新请求的 loading；离开页面或重建表单时按真实生命周期失效旧结果。
- 草稿与服务器已保存事实分开。测试连接、发布或其它动作究竟消费草稿还是已保存配置，必须来自正式 Contract；只支持已保存配置时明确提示/约束 dirty 状态，不能悄悄保存或让用户误以为测试了当前草稿。
- 保存失败保留用户输入并可重试；成功后使用服务端返回或正式重读的规范化值、版本和新 ID 更新持久基线。请求期间允许继续编辑时，旧保存响应不得覆盖更新的草稿；按既定交互锁定冲突动作或检测草稿代次，不自行发明产品策略。
- 数字控件、可选字段和动态选项按真实类型/空值语义归一化，区分合法零、空值、无效输入和字符串；选项/默认值来自当前权威数据，Figma 示例不是默认业务值。
- 验证按本次风险覆盖乱序响应、快速切换、重复动作、保存失败和规范化成功结果。优先在既有 Component/Browser Mock 层制造可控时序，只为真实接线缺口增加 Integration/Golden Path，不把全部状态搬到昂贵全栈测试。
