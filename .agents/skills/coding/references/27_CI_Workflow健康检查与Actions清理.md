...
**少跑 ≠ 允许永久冗余**。每次实现/交付检查 test/test group/CI step/Job/Workflow/build-smoke-install 资产，按 **Owner / Contract / failure boundary / Evidence level** 判断，不按数量。本次新引入、扩大、直接触及或实际暴露的重复/职责混杂资产，若能证明删除、合并或拆责不降低 Required Evidence，必须在 PR Ready / merge 前清理；**不能只通过 selector、skip 或条件判断把永久冗余隐藏起来**。不同平台、Evidence level、权限/生命周期、真实依赖或 required-check identity 不因表面相似合并。**无直接因果关系的历史冗余**只记 Finding，不扩大 Scope。状态：`clean / not_applicable / blocked`；`blocked` 不得宣称 Ready/mergeable/releasable。
