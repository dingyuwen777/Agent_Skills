...
**少跑 ≠ 允许永久冗余**。每次实现/交付将 test/test group/CI step/Job/Workflow/build-smoke-install 按 **Owner / Contract / failure boundary / Evidence level** 判定。本次新引入、扩大、直接触及或实际暴露的冗余，若清理不降 Required Evidence，PR Ready/merge 前须删、并或拆责；**不能只通过 selector、skip 或条件判断把永久冗余隐藏起来**。平台/Evidence/权限生命周期/真实依赖/required-check identity 不同则保留；**无直接因果关系的历史冗余**只记 Finding。状态：`clean / not_applicable / blocked`；blocked 不得交付。
