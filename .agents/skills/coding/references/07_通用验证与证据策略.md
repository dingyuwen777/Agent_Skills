<!-- agent-routing:v1
{"协议":"Agent Skills Reference路由/v1","标识":"coding.reference.07","触发":{"任一":[{"包含":{"维度":"风险","取值":["L2","L3"]}},{"包含":{"维度":"治理","取值":["CI 变更","要求完成门禁"]}}]},"依赖":["coding.reference.02"]}
-->
...
### 1.1 验证下限、默认验证上限与单调升级条件

- **验证下限**：低影响、可逆、边界明确的修改先做直接 targeted validation，**优先复用现有测试**、parser/check、目标命令、readback、diff 或生产入口，取得本轮新鲜 Evidence；不先造第二套测试框架。L1 由 [20_L1轻量实现与验证路径.md](20_L1轻量实现与验证路径.md) 承担，L2/L3 也从同一 targeted-first 起点按真实失败边界扩展。
- **No-New-Test Default**：当改动低影响、可逆、**行为 / Contract 不变**，只是澄清、排版、命名说明或**复述既有实现**，且现有测试/check/readback/diff 已能直接证明目标时，**不新增永久测试**。只有现有 Evidence 无法保护本次真实新增风险、可观察行为、Contract 或具体回归缺口时，才新增最小测试；测试文件数量不是完成指标。
- **默认验证上限**：targeted Evidence 已足够、没有新增可观察行为/Contract/依赖/消费者/独立失败边界、正式门禁或会改变完成结论的具体剩余风险时停止扩大；**不得为了更全面**机械运行无关全仓测试、完整 E2E、所有平台 build/package、Provider Probe 或全量文档审计，也不顺手修邻近技术债。
- **Validation Stop Rule**：与本次改动相称的 Evidence 已通过后，只在出现**新改动、新失败、新独立风险或未解决疑点**时扩大或重复对应验证；否则停止验证扩张并**继续完成任务**。阶段切换、经过时间、换了 Agent、更新 Change/Issue/PR 描述或 Evidence 记录本身，都不是开发侧重复运行同一验证层的理由；项目/PR/CI/Release 的 required current-head gate 仍按真实门禁执行。
- **单调升级条件**：只有 targeted failure、本次新增行为/Contract/状态/依赖/消费者/运行时边界、Review 暴露的新独立失败边界、项目/PR/CI/Release/安全 required gate，或具体未解决风险出现时才升级；扩大时**只增加能直接证明新增风险的下一层 Evidence**，再次评估后再决定是否继续。
- **未知不直接等于更强验证。** 混合 diff、未知路径或事实不足先有界恢复真实消费者、运行时职责和失败边界；只有相关未知经合理调查仍无法排除会影响结论的风险时才升下一层。安全/权限、不可逆数据、真实 package/release 等明确 fail-closed 边界仍执行更强规则。
- 项目既有 required CI / Branch Protection / Release gate 不受这个开发侧上限削弱；targeted-first 不能成为跳过正式门禁的理由。
...
新增可观察行为和 Bug 修复通常需要这一层，除非目标本身无法合理在局部隔离验证。**behavior-preserving refactor** 不因为“重构”标签自动新增测试：先**优先复用已有直接回归**、现有行为测试和必要静态/编译检查；只有 refactor 暴露了尚无证据保护的具体不变量、消费者边界或真实回归缺口时，才补最小测试。

对于需要新失败证据的行为变化，默认 Red → Green → Refactor：
...
现有 [`08_分层测试与验收策略.md`](08_分层测试与验收策略.md) 是专项事实源；本文件不要复制它的全部 Browser/API/PostgreSQL 场景，以避免未来两份细节漂移。
...
11. 是否在 targeted Evidence 已足够后，仍仅因“更全面”无必要扩大验证？
...
验证失败也不自动等于“跑更多测试”。先判断失败属于生产缺陷、测试缺陷、环境/依赖还是本次范围外问题；只有新的独立风险成立时才按 1.1 节增加下一层 Evidence。
...
这条 fast path 不能按扩展名粗判。Markdown、YAML、JSON、文本文件都可能是机器行为事实；例如会被 Runtime、LLM 或生成器消费的 **Prompt**、模板、规则文件、Contract/Schema、Migration、generated artifact、Manifest/锁文件、配置/env、build/startup/deploy、CI/Workflow/Release 文件都不能仅因“看起来像文档”进入轻量路径。

混合 diff、事实不足或**未知路径**不直接等于 `full` / 更强 profile。先按 1.1 节做有界事实恢复，确认真实消费者、是否被机器读取、是否改变 Contract/运行时职责及独立失败边界；只有未知与当前修改相关且合理调查后仍无法排除会影响完成结论的风险时，才升级到能证明该风险的下一层。安全/权限、不可逆数据、真实 package/release 等项目明确 fail-closed 边界除外。
...
full
→ 产品代码、机器消费文件、构建/配置/Workflow、混合或经有界调查仍无法安全分类且会影响完成结论的变化
```

这些名字不是通用固定 API；项目 Overlay 应基于真实路径和消费者映射。轻量分类需要正向事实依据，但“未知”应先调查而不是无条件全量运行；不能通过 `**/*.md`、文件后缀或宽泛目录猜测来降低验证。
...
### Workflow 生命周期与 Actions Hygiene

删除或重命名 GitHub Actions Workflow 文件时，代码 diff 完成不代表 Workflow 生命周期已经收尾。GitHub 可能继续用历史 workflow runs 在 Actions → All workflows 展示已经失效的入口。目标仓库已经建立 repository-native Actions Hygiene 时，应把这类历史收尾作为 Workflow Owner 的长期责任，而不是每次人工维护。

长期 Hygiene 必须满足：

- **只认 Workflow path，不按显示名称猜测**；名称可以变化，`.github/workflows/<file>.yml|yaml` 才是机器身份；
- 当前默认分支仍存在的 path 是绝对保护集合，任何历史 run 都不得删除；
- 只有“当前默认分支已不存在 + 当前 HEAD 祖先历史证明曾真实进入默认分支”的 path 才能成为 obsolete candidate；只在 PR/其他 branch 出现过的 Workflow 默认保留；
- candidate path 只要还有 queued、in_progress、waiting 或其他非 completed run，就整条 path 跳过，等待后续健康默认分支运行重试；
- 先完整分页 repository workflow records 形成稳定候选集合；只对 stale workflow ID 定向完整分页 runs，禁止为了 Hygiene 每次扫描全仓所有 Actions runs；删除后逐 stale workflow ID fresh readback。API、Git 历史或返回结构无法可靠确认时 fail closed；
- destructive 权限必须使用独立 Job 的最小 `actions: write`，不得为了 Hygiene 把整个 CI Workflow 提升为写权限；
- 优先复用仓库现有长期 CI Workflow，并在默认分支健康 Gate 之后运行；不要为了清理 All workflows 反而新增永久 Cleanup Workflow 菜单项；
- Hygiene 是仓库维护能力，不应把 GitHub maintenance API 的暂时波动冒充产品代码失败。仓库可以把远端 API 失败降级为明确 warning 并在后续默认分支运行重试，但脚本自身的候选判定和删除必须 fail closed；
- 历史 run 删除不可逆。若目标项目需要长期保留旧 Workflow 日志作为审计/Release Evidence，必须在该项目 Overlay 明确排除对应 path，而不是通用规则擅自删除。

新增、删除或重命名 Workflow 时，Workflow Responsibility Audit 还要回答：

```text
当前 Workflow 定义
→ 默认分支历史
→ repository workflow records
→ stale workflow ID 定向 runs
→ 是否存在独立 Evidence/审计保留要求
→ Hygiene 是否应清理
```

仓库没有 repository-native Hygiene 时，不为普通 Feature 静默发明 destructive automation；用户明确授权 CI/Workflow 维护时，优先采用“repo-local 可测试脚本 + 现有 CI main-push Job + job-level 最小权限”的最小方案。
