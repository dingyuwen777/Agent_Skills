...
- **验证下限**：低影响、可逆、边界明确的修改先做直接 targeted validation，**优先复用现有测试**、parser/check、目标命令或生产入口，取得本轮新鲜 Evidence；不先造第二套测试框架。L1 由 [20_L1轻量实现与验证路径.md](20_L1轻量实现与验证路径.md) 承担，L2/L3 也从同一 targeted-first 起点按真实失败边界扩展。
