...
验证既要有**验证下限**，也要有**默认验证上限**。模型能力更强、宿主工具更多或“还能多测一点”都不是扩大验证的独立理由。

#### 验证下限

对低影响、可逆、边界明确的小改动，默认先执行最小充分验证：

1. 只检查本次直接受影响的可观察行为、Contract 或治理语义；
2. **优先复用现有测试**、现有 parser/check、目标命令或直接调用，不为一个小修改先创建第二套测试框架/大型 Harness；
3. 取得本轮新鲜 Evidence，并说明它实际证明了什么、不能证明什么。

L1 由 [20_L1轻量实现与验证路径.md](20_L1轻量实现与验证路径.md) 承担；L2/L3 也遵守同一 targeted-first 起点，只是根据真实独立失败边界决定是否需要更强层。

#### 默认验证上限

当直接受影响的 targeted validation 已绿色、没有新增可观察行为/Contract/依赖/消费者、没有发现新的独立失败边界、没有正式项目门禁要求更强 Evidence、也没有会改变当前完成结论的具体剩余风险时，**停止扩大验证范围**。

因此禁止：

- **不得为了更全面**而机械运行与当前 diff 无关的全仓测试、完整 E2E、所有平台 build/package、真实 Provider Probe 或全量文档审计；
- 不因为测试工具已经安装、CI 有某个 job、模型能够枚举更多 edge case，就把它们自动加入开发侧验证；
- 不为一次小修改顺手修邻近技术债、扩大重构或新增与当前风险无关的测试资产；
- 不把“可能还有未知问题”当作无边界继续测试的理由。

项目已经存在的 required CI / Branch Protection / Release gate 仍按项目正式治理执行；这里限制的是 Agent 自行扩大**开发侧验证范围**，不是授权跳过真实门禁。

#### 单调升级条件

只有出现以下任一当前事实时，才扩大验证范围：

- targeted validation 失败，且失败与当前修改或其依赖边界有关；
- 本次实现实际引入新的可观察行为、Contract、状态、依赖、消费者或运行时边界；
- 调查/Review 暴露新的独立失败边界；
- 当前项目、PR、CI、Release 或安全规则明确要求更强 Evidence；
- 仍存在一个具体、可解释、会影响当前完成结论的未解决风险。

扩大时**只增加能直接证明新增风险的下一层 Evidence**，再次评估后再决定是否继续；不得从一个局部未知直接跳到全量 profile。

**未知不直接等于更强验证。** 对混合 diff、未知路径或事实不足，先做有界事实恢复：确认真实消费者、运行时职责和失败边界。只有该未知与当前修改相关，且在合理调查后仍无法排除会影响当前结论的风险时，才升级到能够覆盖它的下一层。安全/权限、不可逆数据、真实 package/release artifact 等项目明确 fail-closed 边界仍按其更强规则执行。
