<!-- agent-routing:v1
{"协议":"Agent Skills Reference路由/v1","标识":"coding.reference.11","触发":{"任一":[{"包含":{"维度":"执行模式","取值":["审查","Git","发布"]}},{"包含":{"维度":"阶段","取值":["审查","交付"]}}]},"依赖":["coding.reference.07","coding.reference.10"]}
-->

# 两阶段复核与完成前验证

本 Reference 负责**正式审查、Ready/Git/Release 交付和项目明确要求的完成前复核**。普通任务只执行 targeted validation、局部回归或为了取得新鲜证据而进入 `执行模式=验证`，**不等于已经触发完整两阶段 Review**；这种验证继续由通用验证规则负责，只有出现审查/交付/项目门禁等真实事实时才加载本 Reference。
...
进入 `ready_for_review` 前：Validation Asset Redundancy=clean / not_applicable;blocked 禁。
...
Review / Testing / Coding 的返修循环不能以“Reviewer 没有任何意见”作为完成标准。进入 Ready / Git / Release 交付前，至少同时满足：

```text
Acceptance Criteria satisfied
+ no unresolved IN_SCOPE_BLOCKING
+ Fresh validation evidence 覆盖当前 revision 和实际影响边界
```

`IN_SCOPE_NON_BLOCKING` / `OUT_OF_SCOPE` 可以在证据和后续处理方式明确时保留；它们不为了制造“零 Finding”而自动扩大当前任务。`REQUIREMENT_CHANGE` 必须按上游决策/授权规则处理。达到自动返修轮次安全阀也**不是**完成证据；仍有 `IN_SCOPE_BLOCKING` 时保持 blocked。
...
每个结论先回答什么命令或检查能证明，并按 Router 的 Fresh Evidence Contract 核验当前证据；独立复核不等于无条件重新执行：

1. 定位实际执行的完整命令、相关 revision、环境与原始输出；仍有效且覆盖当前结论时复用；
2. 只有证据失效、覆盖不足或 required current-head gate 要求时，再运行对应完整命令并读取完整输出；
3. 检查退出码、失败数、警告和跳过项；
4. 对照原始症状、上游完成定义、成功标准、Validation Matrix 和影响范围；
5. 只陈述证据直接支持的状态；没有新改动、失败或未解决疑点时按 Validation Stop Rule 继续收尾。

### 通用证据表

| 结论 | 必需证据 | 不足以证明 |
| --- | --- | --- |
| 行为/Component 测试通过 | 当前目标测试输出、退出码 0、失败数 0，并确实断言目标行为 | 已失效或无法核验的日志、Mock 调用次数、“代码看起来对” |
...
| 子 Agent 完成 | 主 Agent 检查实际 diff 与底层执行证据，按失效/缺口决定是否重跑 | 只有子 Agent 的成功报告 |
