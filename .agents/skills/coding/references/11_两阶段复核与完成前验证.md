...
进入 `ready_for_review` 前按 [CI Workflow 健康检查与 Actions 清理](27_CI_Workflow健康检查与Actions清理.md) 执行；仅 `clean / not_applicable` 可 Ready，`blocked` 禁 Ready/merge，CI Green/skip 不替代。
