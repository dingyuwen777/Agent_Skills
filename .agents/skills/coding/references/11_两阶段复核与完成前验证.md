...
完成以 `Acceptance Criteria` satisfied、无 unresolved `Delivery Effect=BLOCKING`、当前 `Fresh validation evidence` 为准，**不是**“Reviewer 没有任何意见”；轮次安全阀不是 PASS。`OUT_OF_SCOPE + BLOCKING` 可以阻塞当前交付，但这**不授权**把它自动吸收进当前修复 Scope；应按 Router 的 `BLOCK_CURRENT_DELIVERY` 和 Review 的 Follow-up / Requirement 边界处理。
