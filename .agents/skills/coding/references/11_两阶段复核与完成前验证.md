...
进入 `ready_for_review` 前，凡当前任务新增、修改、直接触及或由本轮 CI 实际暴露的 test / test group / CI step / Job / Workflow / build-smoke-install 验证资产，都要按 [CI Workflow 健康检查与 Actions 清理](27_CI_Workflow健康检查与Actions清理.md) 执行 Validation Asset Redundancy Gate。

Completion Audit 只接受 `clean / not_applicable` 进入 Ready：`clean` 表示当前直接相关冗余已清理或已证明每项仍有独立 Evidence 价值；`not_applicable` 必须有“本任务未触及/暴露该边界”的事实依据。已确认 required 冗余但无法安全收口时为 `blocked`，不得用 CI 绿色、selector skip 或“后续优化”替代。
