...
**公开产品私钥是当前明确接受的部署取舍**：Agent_Skills 当前允许 Public Repository 直接跟踪 `licensing/private_key.pem`。这意味着任何读取仓库的人都能用同一产品私钥生成、续期或修改 Claims 后重新签发 Runtime 可接受的 License，因此该部署不能提供 issuer exclusivity、授权防伪造或“用户不能自行延期”的保证。Runtime 仍执行 exact-bytes Ed25519 验签、schema/product/time 检查和固定项目路径期限门禁；这些属于本地一致性/流程控制，不应表述为对恶意用户的授权安全。旧产品 key 是否继续受信任由当前 `licensing/public_key.pem` 决定；切换产品 key 必须通过独立 Change 和三平台 fresh package Evidence。
...
- Cursor 只认领 `.cursor/mcp.json` 的 `mcpServers.agent-skills`，并以唯一 Role Manifest 生成 `.cursor/agents/agent-skills-*.md`；已有 namespaced agent 只有含精确 role ownership marker 才可升级；
- Claude Code 只认领 `.mcp.json` 的同名 server、最薄 `CLAUDE.md` bridge，并以同一 Role Manifest 生成 `.claude/agents/agent-skills-*.md`；已有 namespaced agent 只有含精确 role ownership marker 才可升级；
- Codex 只认领 `.codex/config.toml` 的 Agent Skills managed MCP block，并以同一 Role Manifest 生成 `.codex/agents/agent-skills-*.toml`；已有 namespaced agent 只有含精确 role ownership marker 才可升级；
- DeepSeek Harness 只认领项目 `.dsh/agent-skills.cordis.yml` 的唯一 DeepSeek managed block；该 block 保留 Agent_Skills MCP，并使用当前 DSH base 已提供的 `dsh-subagent`、`dsh-subagent-spawn-in-process`、五个 role `dsh-tool-subagent`、control 与 list-agents 建立 `ctx.subagents` execution surface，不执行 npm/pnpm 在线安装；Windows 只认领项目根 `DeepSeek-Harness.cmd` 的唯一对应 managed block；两个专用文件存在但 marker 缺失/损坏/重复时 fail closed；
...
Runtime managed block 只表达项目侧契约：先读项目自身规则与真实事实，并**只公开稳定 `.agents/skills/ENTRY.md` 作为项目工程约束 Bootstrap**；正常展示真实工程过程，治理能力自身运行/实现细节不作为项目进度或交付内容。详细 Runtime 披露边界由本 canonical Owner 与 Runtime 私有执行/公共输出 Contract 承担，不把 Router、专业 Skill、Reference、内部控制面清单或“防披露说明”复制回目标根 `AGENTS.md`、Entry、Skill Core 或 agent prompt。
...
11. 四 Host 项目配置均使用可移植项目相对语义，不固化安装机器绝对路径；唯一 Role Manifest 确定性生成 Codex/Claude/Cursor 五角色 native agent files 与 DeepSeek role tools；所有 namespaced projection 都有 ownership preflight、幂等升级和事务 rollback；DeepSeek 不修改 `$DSH_HOME`，不联网安装 package；
...
→ 稳定 `.agents/skills/ENTRY.md` Bootstrap
