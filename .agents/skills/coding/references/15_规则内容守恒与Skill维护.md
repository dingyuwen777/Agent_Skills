...
4. **摘要 / 精简 / 压缩不是删除约束的授权。** 逐项保留 `触发条件 / 适用范围 / 前置条件`、规则强度、例外、`失败 / 停止处理`、`Owner / Contract / 数据与 Migration 边界`、`验证责任 / Evidence / 完成判据`、安全/兼容/回滚，以及`跨 Skill / Reference 的触发与回程路径`；不能用一条抽象原则替代多条会影响执行的条件式规则；
