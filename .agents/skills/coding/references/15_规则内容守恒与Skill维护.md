...
### 1.1 摘要、压缩与上下文预算不得吞掉约束

**摘要 / 精简 / 压缩不是删除约束的授权。** 无论目的是降低 context budget、减少文件长度、做 progressive disclosure、生成“更简洁”的规则，还是把多段内容合并成摘要，都必须先逐项盘点并保留原规则的规范语义，包括：
...
1. 摘要可以改变表达方式，不能改变规范强度、逻辑关系和责任边界；多条并列、条件式、例外式或失败处理规则，只要差异会影响执行，就不能合并成一句泛化结论。
2. context budget 超限时，优先消除**可证明完全等价**的重复、复用既有 canonical Owner、改进渐进披露/路由或把详细规则移动到仍会在执行前命中的 Owner；不得删除约束、抬高预算阈值或放宽测试来制造 Green。
3. Core 可以更薄，但只有在对应详细 canonical 规则仍完整可达、触发条件仍明确、并能在相关动作前加载时，才允许用导航/摘要替代重复正文。
4. 任何“总结版”“精简版”“重写版”要替代 canonical 规则时，都必须做 old → new 的逐项语义对照；无法证明语义等价时，保留原文细节，而不是接受“意思差不多”。
5. 文件更短、token 更少、摘要更顺滑都不是成功标准；成功标准是约束完整、触发可达、责任未降级、验证仍能直接证明。
