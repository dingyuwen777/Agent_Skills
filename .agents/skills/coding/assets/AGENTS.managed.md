...
6. 对跨多阶段或可能发生上下文压缩、任务交接、模型切换、宿主重连的长任务，在阶段实质变化或交接前保留最小任务状态：目标/成功标准、已确认决定、已完成切片及证据、当前前沿、阻塞项、失败假设、未验证风险、下一步和非目标；恢复后先按当前仓库事实重新核对再继续。该状态不产生权限，也不能替代测试、Review、CI 或完成证据。
