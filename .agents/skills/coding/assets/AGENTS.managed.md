...
3. 在计划前执行多 Agent 价值判断：`NO_SPLIT` 保持单 Agent；明显低风险、明显 `NO_SPLIT` 的小任务直接正常处理，不单独播报编排 banner；`MAY_SPLIT` / `MUST_SPLIT`、实际拆分或因宿主能力 fallback 时再向用户说明。只拆具备独立目标、独立输出/证据或明确隔离价值的工作。
...
5. Parent 集成 child 结果前核对当前 revision 和已确认决定；基于旧 revision/旧决定的结果必须重新验证，不能直接驱动修改或完成。child 对同一 transient 失败只做有限重试，重复失败回 Parent fallback/replan；Evidence 已充分或 child 已无价值时停止等待，并在宿主支持时取消。`OUT_OF_SCOPE` 默认只记录，**不自动创建**后续 Issue/任务，也不自动执行或递归派生。
6. 当前宿主没有可用 subagent/delegation、该能力被禁用或无法安全使用时，明确说明后**降级为单 Agent正常执行**；不得因此阻塞本可完成的工程任务，也不得伪称已经拆分。项目自身已有的独立 Review、权限、安全、CI 等硬门禁仍照常生效。
