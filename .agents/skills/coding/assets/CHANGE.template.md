...
从用户已确认决定、正式路线图/规格/阶段/功能完成定义、新建项目正式需求/约束或其他上游事实源独立提取要求。**当前 Change 不能把自身作为需求来源，也不能把本表当作上游需求全集。**
...
`来源` 优先写仓库相对事实源路径；本轮用户明确决定可写 `user:<简短标识>`；外部正式资料可写 `external:<可识别来源>` 或 URL。`证据` 必须写实际实现、测试、运行或正式延期/不适用依据，就绪时不得保留占位内容。
...
| 接口 / 契约 | not_applicable | 公共 API/ABI/CLI/Schema/格式/生产者消费者边界受影响时验证兼容和机器一致性 |
| 集成 / 持久化 / 运行依赖 | not_applicable | 真实数据库、文件、队列、操作系统、SDK、运行服务等语义受影响时验证真实依赖边界 |
| 用户 / 工作流验收 | not_applicable | 有用户/调用者可观察工作流时验证入口、输入、输出、状态和错误闭环 |
| 跨组件关键路径 | not_applicable | 存在多个真实组件的关键接线时，用少量高价值路径证明组装后的真实链路 |
| 外部依赖 / 供应方探测 | not_applicable | 只有需要确认第三方服务、硬件或远端环境当前真实事实时才有界执行 |
| 构建 / 打包 / 运行 | not_applicable | 构建、打包、安装、镜像、目标平台或启动行为可能受影响时验证正式产物/运行入口 |
| 文档 / 治理 / 其他 | not_applicable | 文档、配置、生成物、架构/Owner、Secret、Policy、Change/就绪等专项证据 |

通用规则见 [`.agents/skills/coding/references/07_通用验证与证据策略.md`](../../../skills/coding/references/07_通用验证与证据策略.md)。

项目存在界面/API/持久化/外部依赖专项边界时，在保持语义责任不变的前提下按 [`.agents/skills/coding/references/08_分层测试与验收策略.md`](../../../skills/coding/references/08_分层测试与验收策略.md) 映射为更具体层名，例如：
...
项目实际使用 PostgreSQL、MySQL、SQL Server、SQLite、文件系统、DynamoDB 等具体持久化方式时，集成验证必须证明对应真实语义；浏览器/界面模拟不能冒充真实后端/持久化；一条关键路径不能冒充全部状态；真实外部探测默认有界且不进入普通 CI。

# 完成审计

进入 `ready_for_review` 前必须**重新读取上游事实源**，不要从当前 Change 的检查表反推需求。

按当前项目形态和任务边界执行正向/反向审计。例如：

- 前后端：后端能力 → 前端入口，前端动作 → 后端真实能力；
- CLI：公共命令/参数 → 处理器 → 标准输出/标准错误/退出码/副作用；
- Library：公共 API → 消费者；
- 异步：请求 → 状态 → 错误/恢复 → 最终结果；
- Schema/迁移：写入方 → 迁移 → 读取方/消费者；
- 打包/发布：源码 → 构建产物 → 安装/启动；
- 基础设施：配置 → 计划/渲染 → 运行/部署边界（在授权范围内）；
- 新建项目：目标/硬约束 → 工程基线 → 构建/测试/打包/启动 → 最小真实用户/消费者结果。
...
- [ ] reverse_audit：已执行适用的反向能力/边界审计，并复核验证矩阵；不适用项已有明确依据。
