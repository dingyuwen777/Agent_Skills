...
Coding Handoff 前必须集中输出 `Capability Gap Inventory`：本次审查范围内全部已发现缺口按同一真实能力 / Owner 去重并沿用 6.1–6.3。每项至少含：Figma 位置 / 用户能力 / 当前系统证据 / 正式 Owner / 缺失或冲突层（不存在则 not_applicable） / 分类 / 阻塞性 / 最小动作。不得等到 Coding 实施中途才首次披露。当前任务已经明确批准把范围扩大到真实跨层实现→Coding，否则禁伪实现/永久 mock。没有能力缺口时也必须明确输出 `none`。
