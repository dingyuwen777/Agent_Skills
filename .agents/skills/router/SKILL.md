...
| Skill | 职责 | 入口 |
| --- | --- | --- |
| `router` | 路由/Handoff | [`.agents/skills/router/SKILL.md`](SKILL.md) |
| `coding` | 研发/Git | [`.agents/skills/coding/SKILL.md`](../coding/SKILL.md) |
| `testing` | 测试 | [`.agents/skills/testing/SKILL.md`](../testing/SKILL.md) |
| `review` | 独立审查 | [`.agents/skills/review/SKILL.md`](../review/SKILL.md) |
| `docs` | 文档 | [`.agents/skills/docs/SKILL.md`](../docs/SKILL.md) |
| `figma` | 设计/Ready | [`.agents/skills/figma/SKILL.md`](../figma/SKILL.md) |

Runtime/Project Payload/manifest/测试/Release 继续动态发现；Review 只判充分性，测试方法归 Testing。

## 3. Owner-gated 固定入口

1. 恢复最少充分事实；
2. 按任务对象/专业意图选 Owner；项目形态、风险、工具链、范围、治理、授权、Capability 只细化已命中 Owner；
...
| Figma → Code | Handoff | Figma + Coding | `执行模式=实现；意图=设计转代码；范围=前端` |
...
- 不适用：普通业务。
- 交接：[`12_目标项目安装与AGENTS_Bootstrap.md`](../coding/references/12_目标项目安装与AGENTS_Bootstrap.md)（`coding.reference.13`）→ Runtime 再读 [`13_本地MCP_Runtime分发与原文上下文加载.md`](../coding/references/13_本地MCP_Runtime分发与原文上下文加载.md)（`coding.reference.14`）。
- 返回：smoke → Coding 验证/Review/Git。
- 失败关闭：关键事实**无法读取**/验证则阻塞依赖动作，**不得假装**完成。

## 7. Figma 路由

- 触发：Figma 创建/修改/审查/Prototype/基线/Design-to-Code。
- 必须动作：读取 [`.agents/skills/figma/SKILL.md`](../figma/SKILL.md)；review-only 不要求 `READY`，baseline-ready 才输出 **READY / READY_WITH_NOTES / NOT_READY**。
- 不适用：无 Figma 事实。
- 交接：设计 → Figma；生产实现 → Coding。
- 返回：Review → Findings；正式基线 Ready 后按需 Coding/Testing/Review。
- 失败关闭：缺 Figma/required Context 时不冒充审查/Ready；其他无依赖工作继续。
...
- 触发：Code Review/Audit、专业独立 Review 或项目门禁；Figma/Docs“审查”不自动成为 Code Review。
...
Router 只拥有发现、Owner-gated 加载和 Handoff；专业细节归 Coding/Testing/Review/Docs/Figma/Runtime。不能把专业细则复制回 Router/ENTRY/managed block。