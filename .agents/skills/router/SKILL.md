...
**Cross-Skill Terminal / Handoff Contract**：`HANDOFF_CURRENT_SCOPE / REPORT_ONLY / BLOCK_CURRENT_DELIVERY / REQUIREMENT_DECISION / FOLLOW_UP_CANDIDATE / STALE_RESULT / CAPABILITY_BLOCKER`；`OUT_OF_SCOPE + BLOCKING` 只阻塞不扩 Scope；FOLLOW_UP_CANDIDATE 不自动执行/递归，持久化另需授权。
