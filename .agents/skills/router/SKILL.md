...
本 Skill 是 Agent_Skills **唯一的跨 Skill Catalog / Router 事实源**。项目事实归目标项目，专业方法归各 Skill；Router **只输出** Skill、required References、最低风险、Handoff 和失败边界。
...
- **Model-neutral Execution Contract**：模型/Provider/版本只作为 Eval/Trace 标签，**不是 Router 维度，也不改变治理强度**。同一项目事实、风险、授权和 Requested Outcome 必须使用同一 canonical Skill/Reference、Evidence 与 Completion Contract；宿主真实工具能力差异继续使用已有的 能力/工具链/授权 事实细化。不得因为 GPT、DeepSeek、GLM 或其他模型“更强/更弱”新增品牌专属治理分支或降低 required gate；真实效果差异用 [跨模型一致性与 Agent Outcome Eval](../coding/references/31_跨模型一致性与Agent效果评测.md) 评测。
...
- 必须动作：恢复 installation/ownership/schema/宿主事实；读完整 canonical Reference。
- 不适用：普通业务。
