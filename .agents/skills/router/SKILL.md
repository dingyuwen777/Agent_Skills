...
终态：`HANDOFF_CURRENT_SCOPE / REPORT_ONLY / BLOCK_CURRENT_DELIVERY / REQUIREMENT_DECISION / FOLLOW_UP_CANDIDATE / STALE_RESULT / CAPABILITY_BLOCKER`。仅 HANDOFF_CURRENT_SCOPE 可连续同范围授权；`OUT_OF_SCOPE + BLOCKING` 只阻塞不扩 Scope；FOLLOW_UP_CANDIDATE 不自动持久化/执行/递归，当前 Git/写权限不授权其持久化。
