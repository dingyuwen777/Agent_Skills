...
**每次维护收尾**执行 coding.reference.28 的 Validation Asset Redundancy Gate；**少跑不等于允许永久冗余**。本次直接相关冗余可证等价即在 Ready / merge 前清理，历史无因果只记 Finding；仅 `clean / not_applicable` 可完成，`blocked` 禁 Ready / merge。
