...
**每次维护收尾**按 coding.reference.28 执行 Validation Asset Redundancy Gate：**少跑不等于允许永久冗余**。直接相关且可等价清理的冗余须在 Ready/merge 前收口；历史无关只记 Finding；仅 `clean / not_applicable` 可交付，`blocked` 不得 Ready/mergeable/完成。
