...
源仓库投影 ownership 来自固定 canonical mapping 与原字节一致性，不依赖 ownership marker。目标项目的 Runtime projection ownership 由 Runtime Owner 使用 previous install-state、previous managed canonical bytes 与目标 projection byte equality 证明；证据不足或项目侧 drift 时 fail closed，不新增 governance sidecar，也不提供 force-adopt。
...
1. **Project-facing Plaintext**：安装明文只保留当前项目实际工程规则、风险、授权、验证和交付语义；除稳定 `.agents/skills/ENTRY.md` Bootstrap 外，不泄露内部组织身份、源码导航、私有 routing metadata 或防披露自说明；
...
`Agent Skills Gate` Core 负责：Requirement Source、changed-scope selector、selected semantic tests、必要 compile/CLI smoke 和当前 Change Ready 结果；**Change Ready 必须在 Core 对所有 scope 统一 fail-closed，不能依赖可能被 scope skip 的聚合 Gate**。只有 `package` + Ready/non-draft/main 时才启动对称的 Linux/Windows/macOS `runtime-package` matrix；三平台共同调用 `scripts/runtime_platform_smoke.py` 完成 onefile、self-test、真实 stdio MCP 与项目安装验证。`Runtime Package Gate` 保留稳定 required check identity；selector 已证明 `change_only/governance/content` 时通过 **job-level condition 直接 skipped，分配 0 Runner**，package 时只聚合 Core + package matrix 结果并 fail-closed，**不得再次 checkout/setup Python/重复 ready_check**。
