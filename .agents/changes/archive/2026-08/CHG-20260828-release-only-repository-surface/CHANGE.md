...
| --- | --- | --- | --- | --- |
| R1 | 最终使用者只拿 Release 包，不需要了解 Skill 分发与维护过程 | user:2026-08-28-release-only-distribution | satisfied | `USAGE.md` 是唯一最终用户说明；run #180 先证明仍暴露内部实现，随后收敛；run #181/#182 的 107 个 self-contained tests、Ready Check 和三平台 Job 全绿；main run #183 再次全绿。 |
...
| R5 | Skill 规则重组/删除辅助 README 时不得损失正式规则与路由 | .agents/skills/coding/references/16_规则内容守恒与Skill维护.md | satisfied | run #181/#182 的 107 个自包含测试全绿；Docs/Figma/Frontend/Review/迁移清洁度/Runtime exact-text 等守恒回归全部通过，Figma Canvas/Prototype/Ready 等高价值规则仍可达。 |
| R6 | 删除分发兼容面后仍保持现行 Runtime Bundle/Payload/安装/宿主/回滚能力 | .agents/skills/coding/references/14_本地MCP_Runtime分发与原文上下文加载.md | satisfied | run #181/#182：Linux onefile `status/self-test`、真实 stdio MCP、显式/重复/无参数项目安装、项目内 Runtime smoke、Ready Check 全绿；Windows/macOS onefile 与项目安装均成功；main run #183 再次全绿。 |

# Validation Matrix

| Layer | Required | Scope / Evidence |
| --- | --- | --- |
| 行为 / Unit / Component | required | run #182：107 个 self-contained tests 全绿；覆盖仓库 surface、唯一用户说明、动态 Skill、内容守恒、维护 README 排除、Python helper 保留和 legacy cleanup。 |
...
| 集成 / Persistence / Runtime Dependency | required | run #182/#183 在真实临时项目执行 Project Payload → installer → ownership/AGENTS/宿主配置/Runtime Stub，重复安装和项目内 Runtime MCP smoke 成功。 |
| 用户 / Workflow Acceptance | required | `USAGE.md` 与真实 `install --target`、无参数当前目录安装、`status/self-test` 对齐；Linux/Windows/macOS 项目安装均实际运行。 |
| 跨组件 Golden Path | required | run #182/#183 实际完成 source Skill → encrypted Bundle/Project Payload → onefile → target project Runtime/Stub → stdio MCP canonical load。 |
| External Dependency / Provider Probe | not_applicable | 本 Change 不依赖第三方业务 Provider、真实 Figma 或生产环境；GitHub Release 行为由 workflow contract + PR/main CI 静态/构建证据覆盖。 |
| Build / Package / Runtime | required | run #182 与 main run #183：Linux、Windows、macOS 对应 Runner 的 onefile + 项目安装全部成功；Linux 同时完成真实 MCP 与 Ready Check。 |
| Docs / Governance / Other | required | Docs `full` 受影响域复核、Review A1/A2、四项 Completion Audit 已完成；run #182 PR Ready Check 与 main run #183 再验证均成功。 |
...
- [x] unresolved_cleared：R1-R6 全部 satisfied；required 技术验证、run #182 PR Ready Check 与 main run #183 已有新鲜证据；仓库可见性作为明确的 Owner 运维前提保留，不冒充本代码 Change 已修改。
...
- [x] 最终治理快照通过永久 CI，PR #17 已转 Ready 并正常合并
- [x] main merge commit 已完成新鲜 CI，Change 按仓库流程归档
...
- Final Ready：run #182 最终 PR HEAD 三 Job 全绿。
- Main Verification：run #183 merge commit 三 Job全绿。
...
- Final governance Head：`c4a398fd510746b700c9b96b2b9eff14d2782a3e`
- PR：#17，已正常合并
- Merge commit：`440538d946da451ece0cd0ab89ca41d099361dd3`
- Final PR CI：run #182（`33188697941`），三 Job 全部 success
- Main CI：run #183（`33188925117`），三 Job 全部 success
- Archive Branch：`chore/archive-release-only-repository-surface`
- Release：本任务未创建正式版本；只修改下一次正式 Release 的资产/说明合同
