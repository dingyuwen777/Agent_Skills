...
| 编号 | 要求 | 来源 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| R1 | 已有页面前端实现存在时，根据最新 Figma 做差异驱动增量更新，而不是忽略现有正确实现或默认整页重写 | https://github.com/dingyuwen777/Agent_Skills/issues/81 | satisfied | `figma.reference.05` 增加 Existing Implementation Delta Gate；Coding reference 17 增加已有页面差异驱动回程 Contract；final-head run 33327598909 与 main fresh run 33327687783 的 221 个 self-contained tests 全通过。 |
| R2 | 后端真实接口/Contract 与前端或 Figma 不一致时，按真实机器 Owner 修正并同步 Figma Annotation | https://github.com/dingyuwen777/Agent_Skills/issues/81 | satisfied | `figma.reference.02` 增加 Backend / Contract → Annotation Sync，明确“后端符合正式 Contract”与“后端违反正式 Contract/已批准需求”的分支、Pending Figma Sync 和真实消费链；final-head/main fresh CI 全绿。 |
| R3 | 前端实现与原 Figma 发生已确认的长期偏移时，可以同步回 Figma，保持设计和生产实现一致，但不能把 Bug 自动反写成设计 | https://github.com/dingyuwen777/Agent_Skills/issues/81 | satisfied | `figma.reference.05` 增加 Bidirectional Design Sync Gate，只有正式长期事实进入 back-sync；偶然偏移/workaround/Bug 返回 Coding；final-head/main fresh CI 全绿。 |
| R4 | 开发完成后必须明确告诉人工 Figma 修改了哪些位置、为什么改、依据什么，并提供人工复核清单 | https://github.com/dingyuwen777/Agent_Skills/issues/81 | satisfied | `figma.reference.05` 与主 Figma Skill 强制输出 Figma Sync & Human Review，包含 File/Page/Section/Frame/Node、Before→After、事实来源、关联实现/Contract、验证证据、Pending Sync、人工复核重点及 Human Review Status；final-head/main fresh CI 全绿。 |
| R5 | 不削弱现有 Figma Ready、真实系统映射、Owner、Canvas/Prototype 和 Coding Handoff，也不把当前后端错误当正式事实 | .agents/MAINTENANCE.md | satisfied | PR #82 final revision-bound review comment 5470432218 对 base `ce3b4adfbf767f035038bfb63368ce201034fb5e` / head `2bcf88b5e42ec77e44388874a0c36b4e521f0f56` 结论 NO_FINDINGS_WITHIN_SCOPE；PR final CI 33327598909 和 main fresh CI 33327687783 全绿。 |

# 验证矩阵

| 验证层 | 是否要求 | 范围 / 证据 |
| --- | --- | --- |
| 行为 / Unit / Component | required | Red run 33326781696 / job 99298230867：compile、CLI smoke 成功，221 tests 中仅新增 4 个规则断言按预期失败；中间 run 33327232309 / job 99299421827：220/221 通过，暴露“保留正确业务行为”表达不够显式；修规则正文后 Green run 33327408999 / job 99299896660：221 tests 全通过；final-head run 33327598909 / job 99300394179 与 main fresh run 33327687783 / job 99300628906 均全绿。 |
...
| 用户 / Workflow Acceptance | required | PR #82 comment 5470422257 完成 A1/A2；final revision-bound re-review comment 5470432218 以 Issue #81 为 resolved Requirement Source，冻结 current base/head 后复核 Existing UI → Figma Delta → Coding → Contract → Figma Back-sync → Human Review Package，无需求漏项。 |
| 跨组件 Golden Path | required | Green/final/main fresh runs 继续通过 Figma routing、READY/NOT_READY→Coding、Bundle/Project Payload、Canvas/Prototype/Owner preservation 回归；Coding 只新增返回 Figma 的 cross-skill Contract。 |
...
| Docs / Governance / Other | required | PR #82 `Requirement-Source: #81` 可解析；final-head Skill Tests run 33327598909 全绿并通过 changed Change Ready Gate；merge commit `b4591f3497252049f7440c45926137096f77cbc1` 后 main fresh run 33327687783 全绿并通过 Active Change 校验。 |

# 完成审计

- [x] upstream_re_read：完成前重新读取 current branch/root AGENTS、Coding/Mutation/Frontend、Review/ref01/ref02/ref03、PR Traceability 规则，并重新读取 Issue #81、PR #82 current base/head 和受影响 Figma Owner；merge 后又读取精确 main `b4591f3497252049f7440c45926137096f77cbc1` 的根 AGENTS，未从旧聊天或作者 checklist 反推当前事实。
- [x] change_coverage：R1–R5 均有唯一 Owner、实现、preservation test、A1/A2 和 final revision-bound Review 证据；主 Figma Skill只暴露高层入口，ref02/ref05 承担详细语义，Coding reference 17 只承担回程 Contract。
...
- [x] unresolved_cleared：R1–R5 无 not_satisfied；required 行为、Workflow Acceptance、跨组件与治理证据均取得，PR final-head 与 main fresh CI 均成功；实现阶段没有未清阻塞。

# 任务

- [x] 从 main `33f577136c8e52fc4c8ef313a975c5719a2f6172` 读取根 AGENTS、Maintenance、Router、Coding/Mutation、Figma Skill/ref02/ref05、Coding Frontend reference 和 Figma preservation tests；随后确认并行归档让 main 前进到 `ce3b4adfbf767f035038bfb63368ce201034fb5e`，且无受影响文件冲突。
...
- [x] 执行 A1/A2、项目中立性、内容守恒和独立 Review：PR #82 comments 5470422257 / 5470432218，Requirement Source resolved，final reviewed base/head 为 `ce3b4adfbf767f035038bfb63368ce201034fb5e` / `2bcf88b5e42ec77e44388874a0c36b4e521f0f56`，NO_FINDINGS_WITHIN_SCOPE。
- [x] final-head fresh CI run 33327598909 全绿后，以 `expected_head_sha=2bcf88b5e42ec77e44388874a0c36b4e521f0f56` 合并 PR #82；merge commit `b4591f3497252049f7440c45926137096f77cbc1`。
- [x] main fresh Skill Tests run 33327687783 / job 99300628906 全绿，Active Change 校验成功。
- [x] 从验证后的 main `b4591f3497252049f7440c45926137096f77cbc1` 创建独立 archive 分支，将 Change 标记 `done` 并移动到 `archive/2026-08/`；archive PR 负责最终清理与关闭 Requirement Issue #81。
...
- Final ready head：PR #82 / head `2bcf88b5e42ec77e44388874a0c36b4e521f0f56` / run 33327598909 / job 99300394179；compile、CLI smoke、221 self-contained tests、changed Change Ready Gate 全部成功。
- Requirement Review：PR #82 comments 5470422257 / 5470432218；Issue #81 resolved；final reviewed_base_sha=`ce3b4adfbf767f035038bfb63368ce201034fb5e`，reviewed_head_sha=`2bcf88b5e42ec77e44388874a0c36b4e521f0f56`；NO_FINDINGS_WITHIN_SCOPE。
- Merge：PR #82 使用 REST merge + `expected_head_sha=2bcf88b5e42ec77e44388874a0c36b4e521f0f56` 成功，merge commit `b4591f3497252049f7440c45926137096f77cbc1`。
- Main fresh CI：run 33327687783 / job 99300628906；compile、CLI smoke、221 self-contained tests、Active Change 验证全部成功。
...
- Implementation PR：#82，已合并
- Implementation merge：`b4591f3497252049f7440c45926137096f77cbc1`
- Main fresh CI：33327687783，成功
- Archive：独立 archive PR 完成后关闭 Issue #81
