...
- [x] 完成功能、三平台 Package/Runtime、Review 与 Completion Audit 后进入 Ready；PR 永久 CI run `33233994199` 在当前实现/Change 内容上三项 Job 全绿，满足合并前门禁。本 Change 不直接重跑或创建正式 Release。
...
| R2 | 保持现有 Release 产品 Contract 不变 | `.github/workflows/release.yml` | satisfied | PR diff 只修改三平台 staging/upload path；run `33233994199` 中 Release productization 既有 Contract 测试继续通过；最终汇总目录、三平台文件名、`USAGE.md`、`SHA256SUMS`、VERSION/tag 与不可覆盖逻辑均未改变。 |
| R3 | Bug 修复需有回归测试并验证 Red→Green | `.agents/skills/coding/references/05_设计实施与根因调试.md` | satisfied | Red run `33233059615`：132 tests 中仅新增 `test_release_upload_sources_are_not_hidden` 失败，明确命中 `.release-assets`；最终 Green run `33233994199`：132 tests 全部通过，包括该回归测试。 |

# Validation Matrix

| Layer | Required | Scope / Evidence |
| --- | --- | --- |
| 行为 / Unit / Component | required | Red run `33233059615` 精确复现新增回归断言失败；最终 Green run `33233994199` 的 132 个 self-contained tests 全部通过，新回归测试证明三平台 staging 与 upload path 不再使用隐藏目录。 |
| 接口 / Contract | required | run `33233994199` 中 Release productization 既有 Contract 测试继续通过；PR diff 复核确认三个 binary 名称、`USAGE.md`、`SHA256SUMS`、手工 main Release、VERSION/tag 和不可覆盖语义不变。 |
...
| 用户 / Workflow Acceptance | required | 原 Release run `33232571749` 提供真实 workflow 失败证据；最终 PR CI `33233994199` 验证修改后的永久 workflow 文件与所有现有 Release contract tests。正式 `workflow_dispatch` 仍只允许 main 且会创建不可变 Release，因此不在 PR 阶段为测试而触发；合并后需由用户按正式流程手工运行。 |
| 跨组件 Golden Path | not_applicable | 本 Change 不改变 Runtime 内部组件接线；run `33233994199` 的 Linux/Windows/macOS 项目安装链继续通过。artifact 上传本身是单一 Workflow handoff 边界，不需要新增另一条应用 Golden Path。 |
...
| Build / Package / Runtime | required | run `33233994199`：Linux onefile build/status/self-test、真实 stdio MCP、项目安装均成功；`Runtime Windows Package` 成功；`Runtime macOS Package` 成功。Runtime `source_digest` / `payload_digest` 与失败 Release 构建保持一致，说明本 Change 未改变 Runtime 内容 Contract。 |
| Docs / Governance / Other | required | 独立 A1/A2、反向审计与代码质量 Review 结论 `NO_FINDINGS_WITHIN_SCOPE`；run `33233994199` 的 `Verify active Coding Change` 成功，三个 PR CI Job 全部 success。 |
...
4. Verify Green：最终 PR CI run `33233994199` 的 132 tests 全部通过；Linux build/self-test/MCP/install 全部通过；Windows/macOS Package 与项目安装 Job 全部通过；Active Change Ready Gate 通过。

# Review

Review Target：PR #29，base `188d174c970eea1676fe64d6923f669eb0f583f6`。生产实现只在 commit `c83aade2f0f71d2928bac523c2b6d9e88def3b36` 修改 workflow；其后提交只更新本 Change 的验证记录，不改变 workflow/test 实现。
...
- 最终 Green CI 继续证明三平台 onefile/package/install 能力，没有证据显示 Runtime 内容发生变化。
...
- branch: `fix/release-artifact-upload`
- PR: `#29` Draft；最终 PR CI run `33233994199` 三个 Job 全部 success，可转 Ready
- merge: 未执行
