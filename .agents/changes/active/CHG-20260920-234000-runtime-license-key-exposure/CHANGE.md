...
| 编号 | 要求 | 来源 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| R1 | 旧泄露 key 不再被新 Runtime 信任 | user: #283 AC4 + post-closure security requirement | satisfied | public_key.pem 已轮换；旧 key 与 current trusted identity 不匹配 |
...
| R5 | 不声称删除 live file 等于清除历史泄露 | external: GitHub repository metadata + public Git history | satisfied | README/runtime canonical 明确旧 key 永久 compromised |
