...
| 编号 | 要求 | 来源 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| R1 | AC1：Acceptance Criteria 是最终完成状态 Owner，使用稳定 AC ID 与显式状态语义 | external:https://github.com/dingyuwen777/Agent_Skills/issues/184#AC1 | satisfied | ref17 `Issue Acceptance Contract` 明确 final Owner、稳定 `AC1 / AC2 / ...` 与 `satisfied / not_applicable / deferred / unresolved`；`test_canonical_contract_owns_acceptance_state_and_evidence_mapping` 在 Green suite 通过。 |
| R2 | AC2：每个 AC 只有在充分、直接且匹配的 Evidence 下才能 satisfied / 打勾 | external:https://github.com/dingyuwen777/Agent_Skills/issues/184#AC2 | satisfied | ref17 `Evidence Sufficiency Gate` 明确同一对象/行为/条件、revision/commit、必要环境和实际证明内容；明确 CI Green、测试存在/数量、PR merge、Change done 不能单独证明 AC；Evidence Sufficiency regression 通过。 |
| R3 | AC3：GitHub Closure Audit 强制逐项 Evidence、checklist 写回、重读、close 后再读，未完成项阻止关闭 | external:https://github.com/dingyuwen777/Agent_Skills/issues/184#AC3 | satisfied | ref17 固定逐条 Requirement→Evidence、`- [ ]`→`- [x]`、写回、重读、close、再读；`test_github_closure_requires_checkbox_writeback_reread_and_closed_confirmation` 通过。 |
| R4 | AC4：Change 直接追踪上游 AC 和 direct Evidence，不创建第二套需求 | external:https://github.com/dingyuwen777/Agent_Skills/issues/184#AC4 | satisfied | ref17 明确 `Change 不创建第二套需求`；本 Change 的每一行直接引用 #184 对应 AC 并逐项记录 Evidence。 |
| R5 | AC5：三类 Issue Form 统一验收标准、稳定 AC 示例和验证要求，同时保留类型专项字段 | external:https://github.com/dingyuwen777/Agent_Skills/issues/184#AC5 | satisfied | 三类 Form 均统一 `acceptance_criteria`、`label: 验收标准`、`AC1` task-list 示例、`validation_requirements / 验证要求` 且 required；Issue Form contract regressions 通过。 |
| R6 | AC6：需要 post-merge evidence 时 PR 禁止提前 auto-close | external:https://github.com/dingyuwen777/Agent_Skills/issues/184#AC6 | satisfied | PR Template 明确需要 post-merge evidence 时不得使用 `Closes / Fixes / Resolves`，只保留 `Requirement-Source`，由 Closure Audit 后关闭；PR-template regression 通过。 |
