...
| 编号 | 要求 | 来源 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| R1 | 修复 Release #17 的 Bundle schema 漂移失败 | https://github.com/dingyuwen777/Agent_Skills/issues/170 | not_satisfied | Release #17 run 33594361245 已稳定复现；待实施 workflow 修复与回归验证 |
| R2 | 不删除有独立证明责任的现有 CI workflow | https://github.com/dingyuwen777/Agent_Skills/issues/170 | satisfied | 当前 main 仅 3 个 workflow；ruleset 21999314 要求 Agent Skills Gate + Runtime Package Gate；Maintenance 明确 Release 另负最终 artifact 责任 |
| R3 | 消除协议版本第二事实源并保留 Release identity 证据 | https://github.com/dingyuwen777/Agent_Skills/issues/170 | not_satisfied | 已完成 Evidence Preservation Mapping；待代码与测试落地 |
| R4 | 不自动发布 v3.2.0 | https://github.com/dingyuwen777/Agent_Skills/issues/170 | satisfied | 本 Change 授权范围只到修复/验证/PR，不执行 Release workflow |

# Validation Matrix

| 验证层 | Required | Scope / Evidence |
| --- | --- | --- |
| Red / TDD | required | Release #17 run `33594361245` 在三平台 build 成功后稳定失败于 `Runtime Bundle schema 不一致`；新增回归测试应在旧 `release.yml` 上因协议字面量存在而失败。 |
| 行为 / Unit / Component | required | 修改后运行 self-contained tests；PR `Agent Skills Gate` 必须 success。 |
| 接口 / Contract | required | 静态回归证明 Release 仍传递并逐字段比较完整 Builder identity，且不再硬编码协议版本。 |
| 集成 / Runtime Dependency | required | workflow changed-scope 由 `Runtime Package Gate` 按当前 classifier 判定并提供本 PR 新鲜证据。 |
| 用户 / Workflow Acceptance | required | PR CI 验证 workflow 语法与门禁；正式 `v3.2.0` 发布不在本 Change 授权范围。 |
...
| Docs / Governance / Other | required | Ready Check、Change、Workflow Responsibility Audit、Evidence Preservation Mapping、独立 Review 与 PR fresh CI。 |
...
- [x] 比较修复方案并选择单一事实源方案。
- [ ] 新增防协议版本硬编码漂移回归测试。
- [ ] 修改 `release.yml`，删除重复协议版本字面量但保留 identity 比较。
- [ ] 运行/读取新鲜 CI 与独立 Review。

# 完成审计

- [ ] upstream_re_read: 已重新读取 Issue #170、Maintenance、Runtime Release canonical 事实和当前 `release.yml`，实现后再复核。
- [ ] change_coverage: 已覆盖失败根因、CI 审计、回归测试、Evidence Preservation Mapping，完成实现后再核对 diff。
- [ ] reverse_audit: 已从 Builder identity → 三平台 outputs → publish identity → artifact SHA → ZIP/Release assets 反向核对，完成实现后再复核。
- [ ] unresolved_cleared: 当前 R1/R3 与实现/验证任务未完成，完成 Green/Review 后清零。
