...
- [ ] Fresh Evidence 可按 revision/environment/Contract/Scope 有界复用，且唯一跨 Skill Contract Owner 保持为 Router，不因专业 Skill 再解释成第二 Owner。
...
- [ ] targeted 回归与仓库 required Skill Tests 在修复 Review Finding 后重新 Green，scope 保持 content/routing，不触发三平台 binary package。

# 范围

- 收敛 Router/Coding 的自主默认、授权连续性和 Completion Scope。
- 建立 Fresh Evidence 的唯一跨 Skill解释，并让专业验证方法继续留在既有 Owner。
...
- [ ] RF1 / Medium：`.agents/skills/testing/SKILL.md` 当前写成“Fresh Evidence 的复用与失效条件由 Coding 的通用验证策略统一定义”，与本 Change 已确立的 Router 唯一 Fresh Evidence Contract Owner 冲突。修复要求：Testing 只引用 Router 的 Fresh Evidence Contract；具体测试层/场景选择仍由 Testing 自己负责。增加精确回归锁住 Owner，先 Verify Red 再 Green。
...
| R3 | Fresh Evidence Contract | #218 / AC3 | not_satisfied | Router Contract 本身已存在且历史回归 Green，但独立 Review 发现 Testing 错误把 Coding 写成复用/失效条件 Owner；RF1 修复与精确 Owner 回归待完成。 |
...
| R11 | targeted + required Skill Tests Green，scope 不扩大 | #218 / AC11 | not_satisfied | 历史 Green：#1258 482/482、#1259 Ready Gate Green；当前因 RF1 进入修复循环，需在修复 head 上重新取得 targeted + required Skill Tests Green。 |

# 验证矩阵

| 验证层 | 是否要求 | 范围 / 依据 | 当前证据 |
| --- | --- | --- | --- |
| 行为 / 单元 / 组件 | required | autonomy/mutation/testing/completion targeted regression | 历史 #1255 Verify Red、#1258 Green；RF1 新增精确 Owner regression 待 Red/Green |
| 接口 / 契约 | required | `Agent Skills Skill路由/v1` 的 Mutation Audit/Apply required Context、Source/Runtime parity、machine vocabulary | #1258 routing/metadata/conformance/parity 全绿；RF1 不改 metadata |
...
| 用户 / Workflow Acceptance | required | 代表性 Task Route 证明不同请求终点/授权/Testing Handoff 的可观察路由行为 | 新增回归 + 既有 Testing/Owner 路由回归历史 Green；RF1 只修 Owner 文本一致性 |
| 跨组件 Golden Path | not_applicable | 不改多组件产品运行链 | 无对应边界 |
| 外部依赖 Probe | not_applicable | 不需要第三方当前事实 | 无外部事实依赖 |
| Build / Package / Runtime | not_applicable | 不改 executable/package/platform | 历史 classifier=`content`；RF1 仍只触及 Testing canonical text/test/Change |
| Docs / Governance / Other | required | Change/Requirement/Review/CI/Archive/Closure 与内容守恒审计 | RF1 已由独立 Review 记录；修复后需 re-review 与 current-head CI |

# Completion Audit

- [x] upstream_re_read：2026-09-05 在进入上一轮 Ready 前重新读取 live Requirement Source #218；AC1–AC11 未漂移，当前 main 当时仍为 `9b02f35042a277d09ec207d9f65f046fb68fec65`。本轮修复前再次确认 #218 仍 open、main 仍未漂移。
- [x] change_coverage：AC1–AC11 分别映射 R1–R11；RF1 只落在既有 Testing/Fresh Evidence 范围，不增加 Runtime/Workflow/依赖/package 路径。
- [x] reverse_audit：独立 Review 从当前 diff 反查 Rule → professional Skill wording，发现 RF1；这正是反向审计的有效结果。历史 Router/Mutation/Testing routing/content preservation 已由 #1258/#1259 证明，但 RF1 修复后必须重新 re-review。
- [ ] unresolved_cleared：RF1 尚未修复；R3/R11 当前为 not_satisfied。完成精确 Red→Green、current-head CI 与独立 re-review 后再勾选。
