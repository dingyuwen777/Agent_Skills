...
Requirement Source：`user:2026-09-01-local-hardened-runtime-v3-final-plan`。
...
4. 正常 Task Route 的 required Context、canonical exact-text、MCP Tool、Project Payload、安装/升级和宿主使用效果不得退化。
5. 修复当前 `未知项 != []` 无条件将全部 Reference 设为 required 的 full-corpus plaintext oracle。
6. Runtime MCP 不得成为按 ID、文件名、路径、Catalog 或过宽 Task Route 批量导出 canonical corpus 的接口。
...
- [ ] 对 `未知项=[]` 的现有 Routing Conformance，required Context 与既有行为保持一致；unknown routing 不再无条件全库。
- [ ] unknown 导致无法建立最少充分 Context 且最终扩张为 full corpus 时 fail closed，不返回全库原文。
- [ ] MCP 仍恰好六个公开 Tool，不新增 manifest/list/get-by-id/path/filename/glob/dump 接口。
...
- [ ] Linux / Windows / macOS onefile build、status/self-test、真实 stdio MCP、首次安装、重复安装和 v2-installed → v3 upgrade 有新鲜证据。
- [ ] Runtime Package / Skill / Release 独立证明责任不降低；Deep Review 无未解决 BLOCKER/HIGH/MEDIUM。
...
→ full-corpus unknown fail closed
...
- 新 Runtime 只支持 Bundle v3；旧安装 Runtime 自己理解 Bundle v2，并通过现有内部 install-state 提供 previous ownership，新 Runtime 不保留永久 v2 reader。
- 手工回滚使用目标旧版本完整平台 ZIP 和其正式 installer；不在 v3 Runtime 内置 Bundle v2 fallback。
- Project Payload v2、sidecarless ownership、AGENTS/Host managed 边界和安装事务/回滚规则继续保持。

# Requirement Traceability

| 编号 | 要求 | 来源 | 状态 | 证据 |
| --- | --- | --- | --- | --- |
| R1 | Bundle v3 per-reference authenticated encryption + encrypted private manifest | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 待实现与 tamper/record-swap 回归。 |
| R2 | Runtime 不预解密/缓存全库 plaintext，只按 required Context lazy decrypt | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 待 lazy-decrypt instrumentation 回归。 |
| R3 | 正常确定 Task Route 与 canonical exact-text 使用效果不回归 | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 待 routing differential/conformance + exact-text。 |
| R4 | unknown route 不再无条件 full corpus；full-corpus unknown fail closed | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 当前 main 可复现旧行为，待 Red/Green。 |
| R5 | MCP 不提供任意 corpus 导出接口，task capability 拒绝 stale/cross-task/伪造 token | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 待真实 SDK smoke/attack cases。 |
| R6 | Runtime Mode 不输出/高保真重建治理原文，但正常工程解释保持 | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 待 disclosure static + host acceptance。 |
| R7 | Project Payload v2、无 Reference/Stub/sidecar、Host/安装/升级保持 | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 待三平台真实 package/install/upgrade。 |
| R8 | 准确保留 Local Runtime 安全边界，不宣称抵御本机 Owner | user:2026-09-01-local-hardened-runtime-v3-final-plan | not_satisfied | 待 canonical Runtime/Maintenance/README 同步与 Review。 |

# Validation Matrix

| 验证层 | Required | Scope / Evidence |
| --- | --- | --- |
| Red / TDD | required | 复现 unknown→full corpus、全库 plaintext store、v3 tamper/capability 失败用例。 |
| 行为 / Unit / Component | required | AEAD/HKDF/manifest/record/tri-state evaluator/capability/lazy loader。 |
| 接口 / Contract | required | MCP v3 六 Tool、Task Route 公共 schema、Context envelope、Project Payload v2、确定路由 parity。 |
| 集成 / Runtime Dependency | required | RuntimeStore/embedded material/stdio process、old Runtime install-state upgrade。 |
| 用户 / Workflow Acceptance | required | 真实 artifact CLI/MCP/项目安装；Runtime disclosure host acceptance 在可用宿主执行。 |
| 跨组件 Golden Path | required | Host→stdio MCP→route→required exact text→checkpoint + project install。 |
| 外部依赖 Probe | not_applicable | 本次不新增第三方在线 Provider；GitHub Actions 仅作正式 CI/平台证据。 |
| Build / Package / Runtime | required | Linux/Windows/macOS PyInstaller onefile + status/self-test/MCP/install/reinstall/upgrade。 |
| Docs / Governance / Other | required | Change、Runtime canonical Reference、Maintenance/Entry/README、CI responsibility、Deep Review。 |
...
Re-verify
→ self-contained tests + real MCP + three-platform package/upgrade
```
...
待进入 `ready_for_review` 前执行。必须覆盖 AEAD/HKDF/domain separation/record swap、plaintext lifetime、MCP export surface、routing parity、unknown completeness、Project Payload/install/upgrade、disclosure 与三平台 package 证据。
