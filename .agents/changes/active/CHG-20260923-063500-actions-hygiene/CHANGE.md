...
1. 新增 .github/scripts/actions_hygiene.py：扫描当前 workflow paths、Git main 祖先历史和 Actions runs，生成安全删除计划。
2. 脚本默认 dry-run；--execute 才对 eligible completed runs 调用 DELETE，随后 fresh readback。
