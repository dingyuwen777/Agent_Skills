...
- [ ] 新增正式 Release workflow：只在 `main` 的 `VERSION` 变化或人工 dispatch 时执行；Linux/Windows/macOS 分别构建并验证 Runtime Kit，Linux 构建 Full Kit，汇总生成 `SHA256SUMS`，在当前 main SHA 创建不可覆盖的 `vX.Y.Z` GitHub Release。
- [ ] Release workflow 使用固定 commit SHA 的官方 Actions，Release job 仅在全部构建/测试成功后获得 `contents: write`；已有同版本 tag/release 时拒绝覆盖。
- [ ] README、Runtime Distribution、RELEASING、CHANGELOG 与实际版本、资产名称、安装/升级/回滚流程一致。
...
- VERSION、Full Kit Builder、Release 文档与三平台 Release workflow。
...
首个正式产品化版本采用 SemVer `1.0.0`，Git tag 为 `v1.0.0`。正式资产至少包括：
...
Release workflow 不直接发布 PR HEAD，只在合并后的 `main` 上运行；构建 job 全绿后才允许 Release job 创建 tag/release。
...
| R3 | 完成正式 GitHub Release 产品化 | user:current-request | not_satisfied | 待 VERSION、Full Kit、三平台 Release workflow、首个 Release 证据 |
...
| --- | --- | --- |
| 行为 / Unit / Component | required | Coding 内容守恒、安装器 descendant、VERSION/full kit、runtime release_version 测试 |
| 接口 / Contract | required | Full Kit schema、VERSION/tag/asset naming、Runtime manifest/kit additive field、公开 Builder CLI |
...
| 用户 / Workflow Acceptance | required | Release Full Kit / Runtime Kit 解压后的真实安装工作流；README/RELEASING 命令与产物一致 |
| 跨组件 Golden Path | required | main VERSION → 三平台 build → artifact 汇总 → checksums → tag/release → Release asset 可下载/校验/安装 |
...
- [ ] change_coverage：逐条核对 R1-R7，特别检查 Coding 原规则是否有任何不可达或语义降级。
- [ ] reverse_audit：从每个原主规则段落反向找到新位置/触发入口；从 Release 下载资产反向追 VERSION/build/checksum/tag/main SHA。
...
5. 增加三平台 Release workflow 与 checksums/真实 Release 验证。
6. 更新永久 Skill Tests、README、AGENTS。
7. 执行自包含测试、Linux/Windows/macOS package、A1/A2 Independent Review、Completion Audit。
8. 全绿后 PR 合并 main，等待 Release workflow 生成 `v1.0.0`，真实校验 tag、assets、SHA256SUMS 和 Release 安装工作流。
9. main 新鲜 CI + Release 成功后通过独立 PR 归档本 Change。

# 文档影响

- README：版本化 Release 资产和两种分发模式。
- `RELEASING.md`：维护者版本发布、失败/重跑/回滚。
...
- Branch：`feature/release-productization-skill-structure`。
- PR：待实现与验证后创建。
- Release：待 main 合并后由正式 workflow 创建 `v1.0.0`。
